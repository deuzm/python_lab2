...
import pytest
import serializer.json_serializer

from serializer.json_serializer.json_serializer import *
...
@pytest.mark.parametrize("module, expected",
                         [
                             (serializer.json_serializer.mappers, {'module_type': 'serializer.json_serializer.mappers'}),
                         ]
                         )
def test_module_to_dict(module, expected):
    assert module_to_dict(serializer.json_serializer.mappers) == expected


@pytest.mark.parametrize("dct",
                         [
                             ({'module_type': 'serializer.json_serializer.mappers'}),
                         ]
                         )
...
def test_cell():
    assert isinstance(dict_to_cell({ "a": 3}), types.CellType)
